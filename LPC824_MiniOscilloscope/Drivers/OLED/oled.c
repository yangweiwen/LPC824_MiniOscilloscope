...
void OLED_DrawGraduate(u8 x, u8 y)
{
...
void OLED_ShowGrid(uint32_t waveOrFFt)
{
    ...
    //菜单显示
    pVoltName = (u8*)g_scopeCtrl.pVoltName[g_scopeCtrl.voltScaleIdx];
    OLED_ShowString(0,56,pVoltName,8,0);

    pTimeName = (u8*)g_scopeCtrl.pTimeName[g_scopeCtrl.timeScaleIdx];
    OLED_ShowString(5*6,56,pTimeName,8,0);
    
    if(g_scopeCtrl.channelSelect == 0)
        OLED_ShowString(10*6,56,(u8*)"ch0",8,0);
    else
        OLED_ShowString(10*6,56,(u8*)"ch1",8,0);

    if(g_scopeCtrl.runState == SCOPE_STATE_M0_MAIN)
        OLED_ShowString(14*6,56,(u8*)"wav",8,0);
    else
        OLED_ShowString(14*6,56,(u8*)"spm",8,0);
    
    if(g_scopeCtrl.runStop == SCOPE_DISPLAY_RUN)
        OLED_ShowString(18*6,56,(u8*)"run",8,0);
    else
        OLED_ShowString(18*6,56,(u8*)"stp",8,0);
}
...
{ 
	...
	OLED_WR_Byte((y&0x0f)|0x00,OLED_CMD);//LOW
	OLED_WR_Byte(((y&0xf0)>>4)|0x10,OLED_CMD);//HIGHT
}
...
void OLED_ScrollDisplay(u8 num,u8 space,u8 mode)
{
	...
	while(1)
	{
		...
		//	t++;
            OLED_ShowString(8,16,"www.eetree.cn\r\n",16,1);
		}
		if(t==num)
			{
...
void OLED_PIN_Init(void)
{
	...
	Chip_GPIO_PinSetDIR(LPC_GPIO_PORT, 0, 15, 1);  //设置为输出
	Chip_GPIO_PinSetDIR(LPC_GPIO_PORT, 0, 19, 1);  
	Chip_GPIO_PinSetDIR(LPC_GPIO_PORT, 0, 20, 1);  //
	Chip_GPIO_PinSetDIR(LPC_GPIO_PORT, 0, 24, 1);  
}
